...
- [x] T001 プロジェクト構造の確認 - 既存のディレクトリ構造が実装計画と一致していることを確認
- [x] T002 [P] 新規ディレクトリの作成 - `src/logic/` ディレクトリを作成
- [x] T003 [P] 新規テストディレクトリの作成 - `tests/logic/` ディレクトリを作成
...
- [x] T004 [P] 移動パターン型定義の作成 - `src/types/movePattern.ts` にMovePattern型とVector型を定義
- [x] T005 [P] 移動ルール定数の定義 - `src/logic/moveRules.ts` に各駒種のMOVE_PATTERNSを定義
- [x] T006 座標バリデーション関数の実装 - `src/logic/boardState.ts` にisValidPosition関数を実装
...
- [x] T007 [P] [US1] Pieceコンポーネントの選択状態テスト - `tests/components/Piece.test.tsx` にisSelectedプロパティのテストを追加
- [x] T008 [P] [US1] Squareコンポーネントのクリックテスト - `tests/components/Square.test.tsx` にonClickハンドラーのテストを追加
- [x] T009 [US1] ShogiBoardコンポーネントの選択状態管理テスト - `tests/components/ShogiBoard.test.tsx` に駒の選択・選択解除のテストを追加

### Implementation for User Story 1

- [x] T010 [P] [US1] Pieceコンポーネントの拡張 - `src/components/Piece.tsx` にisSelectedプロパティを追加し、選択状態のスタイリング(bg-yellow-200 ring-4 ring-yellow-500)を実装
- [x] T011 [P] [US1] Squareコンポーネントの拡張 - `src/components/Square.tsx` にisSelectedとonClickプロパティを追加し、Pieceへ選択状態を伝達
- [x] T012 [US1] ShogiBoardコンポーネントの状態管理実装 - `src/components/ShogiBoard.tsx` にuseState<Position | null>で選択状態を追加
- [x] T013 [US1] ShogiBoardのクリックハンドラー実装 - `src/components/ShogiBoard.tsx` にhandleSquareClick関数を実装(駒の選択と選択解除)
- [x] T014 [US1] Boardコンポーネントの拡張 - `src/components/Board.tsx` にselectedとonSquareClickプロパティを追加し、Squareへ伝達
- [x] T015 [US1] 基本的な移動機能の実装 - `src/components/ShogiBoard.tsx` で選択中の駒を別のマスに移動する基本機能を実装(ルール検証なし)
...
- [x] T016 [P] [US2] calculateValidMoves関数のテスト - `tests/logic/moveRules.test.ts` を作成し、各駒種の移動可能マス計算のテストを実装(歩、香、桂、銀、金、飛、角、王)
- [x] T017 [P] [US2] isValidMove関数のテスト - `tests/logic/moveRules.test.ts` に移動可能性判定のテストを追加
- [x] T018 [P] [US2] getAdjustedVectors関数のテスト - `tests/logic/moveRules.test.ts` に先手/後手のベクトル調整のテストを追加

### Implementation for User Story 2

- [x] T019 [P] [US2] getAdjustedVectors関数の実装 - `src/logic/moveRules.ts` に先手/後手に応じたベクトル調整関数を実装
- [x] T020 [US2] calculateValidMoves関数の実装 - `src/logic/moveRules.ts` に駒の移動可能マスを計算する関数を実装
- [x] T021 [US2] isValidMove関数の実装 - `src/logic/moveRules.ts` に移動の妥当性を判定する関数を実装
- [x] T022 [US2] ShogiBoardへの移動ルール統合 - `src/components/ShogiBoard.tsx` のhandleSquareClick関数にisValidMoveチェックを追加
- [x] T023 [US2] 移動ルールのエッジケーステスト - 盤面の端、各駒種の特殊な動きをテストで検証
...
- [x] T024 [P] [US3] isPathClear関数のテスト - `tests/logic/collision.test.ts` に経路上の障害物チェックのテストを追加(飛車、角、香の長距離移動)
- [x] T025 [P] [US3] 駒の衝突判定のテスト - `tests/logic/moveRules.test.ts` に移動先に駒がある場合のテストを追加

### Implementation for User Story 3

- [x] T026 [P] [US3] isPathClear関数の実装 - `src/logic/moveRules.ts` に移動経路上の障害物チェック関数を実装(桂馬は経路チェック不要)
- [x] T027 [US3] 移動先の駒チェック追加 - `src/logic/moveRules.ts` のcalculateValidMovesとisValidMoveに移動先の駒の有無チェックを追加
- [x] T028 [US3] 長距離移動駒の経路チェック統合 - 飛車、角、香の移動時にisPathClearを呼び出すよう実装
...
- [x] T029 [P] イミュータブル更新のリファクタリング - `src/logic/boardState.ts` にupdateBoardAfterMove関数を実装し、ShogiBoardから状態更新ロジックを分離
- [x] T030 [P] boardState関数のユニットテスト - `tests/logic/boardState.test.ts` を作成し、イミュータブル更新のテストを実装
- [x] T031 [P] アクセシビリティの向上 - 選択状態の視覚表示にARIAラベルやフォーカス管理を追加
- [x] T032 コードレビューとリファクタリング - 全体のコード品質を確認し、重複を削減
- [x] T033 パフォーマンステスト - SC-001(0.1秒以内の選択表示)とSC-002(0.2秒以内の移動完了)の要件を検証
- [x] T034 quickstart.mdの検証 - 開発環境のセットアップと実装手順が正確であることを確認
